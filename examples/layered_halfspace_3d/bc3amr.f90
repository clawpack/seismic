...
!     # user-specified boundary conditions go here in place of error output
      write(6,*) '*** ERROR *** mthbc(1)=0 and no BCs specified in bc2amr'
      stop
      go to 199
...
!     # enforce sigma.k = 0, extrapolate remaining
    do k=kbeg,nfil
        do j = 1,ncol
            do i=1,nrow
                val(1,i,j,k) =  val(1,i,j,2*kbeg-1-k)
                val(2,i,j,k) =  val(2,i,j,2*kbeg-1-k)
                val(3,i,j,k) = -val(3,i,j,2*kbeg-1-k)
                val(4,i,j,k) =  val(4,i,j,2*kbeg-1-k)
                val(5,i,j,k) = -val(5,i,j,2*kbeg-1-k)
                val(6,i,j,k) = -val(6,i,j,2*kbeg-1-k)
                val(7,i,j,k) =  val(7,i,j,2*kbeg-1-k)
                val(8,i,j,k) =  val(8,i,j,2*kbeg-1-k)
                val(9,i,j,k) =  val(9,i,j,2*kbeg-1-k)
            end do
        end do
    end do
